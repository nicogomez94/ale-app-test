...
const isDueToday = (policy: any) => policy.fechaVencimiento?.slice(0, 10) === format(new Date(), 'yyyy-MM-dd');
const status = (policy: any) => policy.pagada
  ? { label: 'PAGADA', bg: 'rgba(46, 125, 50, .13)', color: '#25843a', text: 'white' }
  : isDueToday(policy)
    ? { label: 'VENCE HOY', bg: 'rgba(255, 157, 0, .16)', color: '#ff9d00', text: '#3b2400' }
  : policy.estado === 'VENCIDA'
    ? { label: 'VENCIDA', bg: 'rgba(220, 45, 45, .13)', color: '#dc2d2d', text: 'white' }
    : policy.estado === 'VENCE_PRONTO'
      ? { label: 'VENCE PRONTO', bg: 'rgba(245, 158, 11, .16)', color: '#f59e0b', text: '#3b2400' }
      : { label: 'VIGENTE', bg: 'rgba(55, 166, 84, .13)', color: '#37a654', text: 'white' };
const clientVisual = (policies: any[]) => {
  ...
  if (relevant.some((policy) => isDueToday(policy) && !policy.pagada)) return { bg: 'rgba(255, 157, 0, .16)', border: '#ff9d00', pulse: true };
  if (relevant.some((policy) => policy.estado === 'VENCIDA' && !policy.pagada)) return { bg: 'rgba(220, 45, 45, .13)', border: '#dc2d2d', pulse: false };
  if (relevant.some((policy) => policy.estado === 'VENCE_PRONTO' && !policy.pagada)) return { bg: 'rgba(245, 158, 11, .16)', border: '#f59e0b', pulse: true };
  return { bg: 'rgba(55, 166, 84, .13)', border: '#37a654', pulse: false };
};
const dueTodayPulseSx = {
  animation: 'clientDueTodayPulse 1.35s ease-in-out infinite',
  transformOrigin: 'center',
  '@keyframes clientDueTodayPulse': {
...
export const ClientsPage: React.FC = () => {
  ...
  return <Box sx={{ minWidth: 0 }}>
    ...
    <Card sx={(theme) => ({ mb: 3, bgcolor: theme.palette.mode === 'light' ? '#fff9e8' : '#29240f', border: '1px solid', borderColor: theme.palette.mode === 'light' ? '#f2dc91' : '#75631b', borderRadius: 3, boxShadow: '0 8px 22px rgba(161,124,20,.08)' })}><CardContent><Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center', mb: birthdays.length ? 2 : 0 }}><Box sx={{ width: 44, height: 44, borderRadius: '50%', bgcolor: '#ffc107', color: '#342500', display: 'grid', placeItems: 'center' }}><Cake /></Box><Box><Typography variant="h6" fontWeight={950}>🎉 Cumpleaños de esta Semana ({birthdays.length})</Typography><Typography variant="body2" sx={(theme) => ({ color: theme.palette.mode === 'light' ? '#765f0d' : '#ffe082' })}>{birthdays.length ? '¡Aprovechá para afianzar el vínculo enviándoles un saludo de cumpleaños por WhatsApp!' : 'No hay cumpleaños cargados para los próximos 7 días.'}</Typography></Box></Box>{birthdays.length > 0 && <Grid container spacing={2}>{birthdays.slice(0, 3).map((item) => <Grid key={item.id} size={{ xs: 12, md: 4 }}><Paper sx={{ p: 2, borderRadius: 3 }}><Typography fontWeight={900}>{item.nombre}</Typography><Typography variant="body2" color="text.secondary">🎂 {item.diasRestantes === 0 ? 'Hoy' : item.diasRestantes === 1 ? 'Mañana' : `En ${item.diasRestantes} días`} · <Box component="span" sx={{ color: 'warning.main', fontWeight: 800 }}>Cumple {item.edad} años</Box></Typography><Button fullWidth size="small" variant="contained" color="success" startIcon={<MessageCircle size={15} />} sx={{ mt: 1.25 }} onClick={() => whatsapp(item, `¡Feliz cumpleaños, ${item.nombre}! Te deseamos un excelente día.`)}>Enviar Saludo WhatsApp</Button></Paper></Grid>)}</Grid>}</CardContent></Card>
